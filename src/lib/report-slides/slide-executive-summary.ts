...
function generate(ctx: SlideContext): void {
  const { pptx, customer, month, fbKpis, igKpis, monthlyAdsData, primaryColor, secondaryColor, pageNumber, imageCache } = ctx;
  const currentAds = monthlyAdsData[2] || monthlyAdsData[monthlyAdsData.length - 1];
  const prevAds = monthlyAdsData[1] || monthlyAdsData[0];
