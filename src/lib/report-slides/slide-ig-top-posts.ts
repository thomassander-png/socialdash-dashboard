...
function generate(ctx: SlideContext): void {
  ...
  const posts = igPosts.filter(p => p.type !== 'VIDEO' && p.type !== 'REEL' && p.type !== 'REELS');
  const topPosts = [...posts]
    .map(p => ({
      ...p,
      interactions: (p.reactions_total || 0) + (p.comments_total || 0) + (p.saves || 0),
      engagementRate: p.reach && p.reach > 0 ? (((p.reactions_total || 0) + (p.comments_total || 0) + (p.saves || 0)) / p.reach * 100) : 0
    }))
  ...
  topPosts.forEach((post, idx) => {
    ...
    if (imageData) {
      try {
        ...
        addTextFallback(slide, post, xPos, startY, imgW, imgH, secondaryColor);
      }
    } else {
      addTextFallback(slide, post, xPos, startY, imgW, imgH, secondaryColor);
    }
    ...
    // Rank badge
    slide.addShape('ellipse', { x: xPos - 0.15, y: startY - 0.15, w: 0.4, h: 0.4, fill: { color: secondaryColor } });
...
function addTextFallback(slide: PptxGenJS.Slide, post: any, x: number, y: number, w: number, h: number, accentColor: string) {
  slide.addShape('rect', {
    x: x, y: y, w: w, h: h,
    fill: { color: 'F5F5F5' },
  ...
  const message = post.message || post.title || post.caption || 'Kein Text verfügbar';
...
export const slideIgTopPosts: SlideModule = {
  id: 'igTopPosts',
  name: 'IG Top Posts',
  description: 'Top 3 Instagram-Beiträge nach Interaktionen mit Bildern',
  platform: 'instagram',
  category: 'content',
  order: 22,
  generate,
